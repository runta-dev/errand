...
const endpoint = (process.env.RUNTA_CREW_E2E_ENDPOINT || "https://api.runta.com").replace(/\/+$/, "");
...
try {
  ...
  const run = await request(`/v1/agents/${encodeURIComponent(agent.id)}/runs`, {
    method: "POST",
  ...
  while (Date.now() < deadline) {
    completed = await request(`/v1/agents/${encodeURIComponent(agent.id)}/runs/${encodeURIComponent(run.id)}`);
  ...
  if (agent?.id) await request(`/v1/agents/${encodeURIComponent(agent.id)}?delete_runtime=true`, { method: "DELETE" });
}
