import { ArrowDown, ChevronDown, ChevronRight, Cloud, FileText, Globe2, LoaderCircle, Monitor, Paperclip, Terminal } from "lucide-react";
...
function WorkingActivity({ agent, label, activities, startedAt }: { agent?: Pick<Agent, "id" | "name">; label: string; activities: ActivityEvent[]; startedAt: string }) {
  ...
  return <details className="agent-working-details" open={open}><summary role="status" aria-label={`${agent?.name ?? "Agent"} is working: ${label}`} onClick={(event) => { event.preventDefault(); setOpen((value) => !value); }}><span className="agent-working-avatar"><AgentAvatar agent={agent ?? { id: "working", name: "Agent" }} size={38} /></span><span className="agent-working-progress">Working for {elapsedLabel(now - Date.parse(startedAt))}</span><ChevronRight className="agent-working-chevron" size={15} /></summary>{activities.length > 0 && <div className="agent-working-tools">{activities.map((activity) => { const Icon = activityIcon[activity.kind]; return <details className={`agent-tool-detail ${activity.status}`} key={activity.id}><summary><Icon size={14} /><span>{activity.title}</span><small>{activity.status === "running" ? "Running" : activity.status === "failed" ? "Failed" : "Done"}</small><ChevronDown size={13} /></summary><div>{activity.output ?? (activity.status === "running" ? "Waiting for result…" : "No output")}</div></details>; })}</div>}</details>;
}
