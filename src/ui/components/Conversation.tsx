...
export function MessageView({ message, agent, activities, entering = false }: { message: Message; agent?: Pick<Agent, "id" | "name">; activities: ActivityEvent[]; entering?: boolean }) {
  ...
  return <div className={`message ${message.role}`} ref={rootRef}>
    ...
    {agentWorking && <div className="agent-working-indicator" role="status" aria-label={`${agent?.name ?? "Agent"} is working: ${workingLabel}`}><span className="agent-working-avatar"><AgentAvatar agent={agent ?? { id: message.conversationId, name: "Agent" }} size={38} /></span><span className="agent-working-progress">{workingLabel}</span></div>}
...
export function Conversation({ agent, messages, activities, loading = false, focusRequest = 0, onSend, onToggleDetails }: { agent: Agent; messages: Message[]; activities: ActivityEvent[]; loading?: boolean; focusRequest?: number; onSend(text: string, attachments?: Attachment[]): Promise<void>; onToggleDetails(): void }) {
  const [draft, setDraft] = useState(""); const [attachments, setAttachments] = useState<Attachment[]>([]); const [sending, setSending] = useState(false); const [isScrolled, setIsScrolled] = useState(false); const [scrollbarVisible, setScrollbarVisible] = useState(false); const scrollRef = useRef<HTMLDivElement>(null); const contentRef = useRef<HTMLDivElement>(null); const composerRef = useRef<HTMLTextAreaElement>(null); const sendingRef = useRef(false); const pinnedToBottomRef = useRef(true); const scrollHideTimer = useRef<number | undefined>(undefined);
  ...
  useEffect(() => { pinnedToBottomRef.current = true; }, [agent.id]);
  useEffect(() => { const element = scrollRef.current; if (pinnedToBottomRef.current && element && typeof element.scrollTo === "function") element.scrollTo({ top: element.scrollHeight, behavior: "smooth" }); }, [messages]);
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => { if (pinnedToBottomRef.current && typeof element.scrollTo === "function") element.scrollTo({ top: element.scrollHeight, behavior: "auto" }); });
    observer.observe(content); return () => observer.disconnect();
  }, [agent.id, loading]);
  useEffect(() => () => { if (scrollHideTimer.current) window.clearTimeout(scrollHideTimer.current); }, []);
