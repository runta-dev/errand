import { CircleStop, Cloud, File, FileText, Globe2, LoaderCircle, Monitor, Paperclip, Terminal, X } from "lucide-react";
...
export function MessageView({ message, agent, activities }: { message: Message; agent?: Pick<Agent, "id" | "name">; activities: ActivityEvent[] }) {
...
export function Conversation({ agent, messages, activities, loading = false, onSend, onToggleDetails }: { agent: Agent; messages: Message[]; activities: ActivityEvent[]; loading?: boolean; onSend(text: string, attachments?: Attachment[]): Promise<void>; onToggleDetails(): void }) {
  const [draft, setDraft] = useState(""); const [attachments, setAttachments] = useState<Attachment[]>([]); const [sending, setSending] = useState(false); const [isScrolled, setIsScrolled] = useState(false); const scrollRef = useRef<HTMLDivElement>(null);
