  ...
  return <div className="dialog-backdrop settings-backdrop" role="presentation"><section className="settings-window" role="dialog" aria-modal="true" aria-label="Settings">
    ...
    <main className="settings-content">
      ...
      <section><h3>Agents</h3><div className="settings-card">{providers.length
        ? <div className="settings-row"><span>Model provider</span><Select ariaLabel="Model provider" placeholder="Select a provider" value={settings.modelProviderId ?? ""} options={[...providers.map((provider) => ({ value: provider.id, label: provider.name })), { value: "__add_provider__", label: "Add model provider", icon: <Plus size={14} />, disabled: !settings.dashboardUrl || !organizationId, action: () => { onModelProviderOpen?.(); addModelProvider(); } }]} onChange={(modelProviderId) => persist({ ...settings, modelProviderId })} /></div>
