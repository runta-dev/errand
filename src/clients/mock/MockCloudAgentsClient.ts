...
import { CrewError, type Agent, type ConversationEvent, type CreateAgentInput, type Message, type ReactToMessageInput, type RespondApprovalInput, type SendMessageInput, type Subscription, type UpdateAgentInput } from "@/domain/types";
...
export class MockCloudAgentsClient implements CloudAgentsClient {
  ...
  async updateAgent(agentId: string, input: UpdateAgentInput, signal?: AbortSignal) { await wait(120, signal); const agent = this.agents.find((item) => item.id === agentId); if (!agent) throw new CrewError("not_found", "Agent not found"); Object.assign(agent, input, input.name ? { avatar: input.name.slice(0, 1).toUpperCase() } : {}); return copy(agent); }
  async deleteAgent(agentId: string, signal?: AbortSignal) { await wait(140, signal); if (!this.agents.some((item) => item.id === agentId)) throw new CrewError("not_found", "Agent not found"); const conversationIds = this.conversations.filter((item) => item.agentId === agentId).map((item) => item.id); this.agents = this.agents.filter((item) => item.id !== agentId); this.conversations = this.conversations.filter((item) => item.agentId !== agentId); this.messages = this.messages.filter((item) => !conversationIds.includes(item.conversationId)); this.approvals = this.approvals.filter((item) => item.agentId !== agentId); this.computers = this.computers.filter((item) => item.agentId !== agentId); }
  async duplicateAgent(agentId: string, signal?: AbortSignal) { const source = await this.getAgent(agentId, signal); return this.createAgent({ name: `${source.name} copy`, role: source.role, goal: source.goal }, signal); }
  async setAgentUnread(agentId: string, unread: boolean, signal?: AbortSignal) { await this.updateAgent(agentId, { }, signal); const stored = this.agents.find((item) => item.id === agentId); if (!stored) throw new CrewError("not_found", "Agent not found"); stored.unreadCount = unread ? Math.max(1, stored.unreadCount) : 0; return copy(stored); }
  ...
  async sendMessage(input: SendMessageInput) {
    await wait(90, input.signal); const message: Message = { id: crypto.randomUUID(), conversationId: input.conversationId, role: "user", parts: [{ type: "text", text: input.text }, ...(input.attachments ?? []).map((attachment) => ({ type: "attachment" as const, attachment }))], createdAt: new Date().toISOString() };
  ...
  async reactToMessage(input: ReactToMessageInput, signal?: AbortSignal) {
    await wait(80, signal); const message = this.messages.find((item) => item.id === input.messageId && item.conversationId === input.conversationId);
    if (!message) throw new CrewError("not_found", "Message not found");
    const existing = message.reactions ?? [];
    const current = existing.find((reaction) => reaction.kind === input.reaction);
    message.reactions = current
      ? existing.map((reaction) => reaction.kind === input.reaction ? { ...reaction, selected: !reaction.selected, count: reaction.selected ? Math.max(0, reaction.count - 1) : reaction.count + 1 } : reaction)
      : [...existing, { kind: input.reaction, count: 1, selected: true }];
    this.emit(input.conversationId, { type: "message.updated", message }); return copy(message);
  }
